...
async def add(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        text = update.message.text.replace("/add ", "", 1).strip().lower()

        if not text.startswith("vless://"):
            await update.message.reply_text("❌ کانفیگ نامعتبره")
            return

        if "1g" in text or "1گیگ" in text:
            plan = "1 گیگ"
        elif "2g" in text or "2گیگ" in text:
            plan = "2 گیگ"
        elif "3g" in text or "3گیگ" in text:
            plan = "3 گیگ"
        elif "4g" in text or "4گیگ" in text:
            plan = "4 گیگ"
        elif "5g" in text or "5گیگ" in text:
            plan = "5 گیگ"
        else:
