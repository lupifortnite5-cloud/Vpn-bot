...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user.id == ADMIN_ID and update.message.reply_to_message:
        ...
        if customer_id:
            await context.bot.send_message(chat_id=customer_id, text=f"✅ کانفیگ VPN شما آماده‌ست:\n\n{text}")
    ...
    if text == "خرید VPN":
        ...
        await update.message.reply_text(f"✅ پلن انتخابی: {text}\n💰 قیمت: {plans[text]}\n\nلطفاً مبلغ رو کارت به کارت کن و بعد عکس رسید رو بفرست.", reply_markup=main_keyboard())
    elif text == "پلن‌ها":
        msg = "📦 لیست پلن‌ها:\n\n"
...
async def handle_media(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user.id == ADMIN_ID and update.message.reply_to_message:
        ...
        customer_id = pending_orders.get(replied_id)
        if customer_id:
            await context.bot.copy_message(chat_id=customer_id, from_chat_id=ADMIN_ID, message_id=update.message.message_id, caption="✅ کانفیگ VPN شما آماده‌ست!")
            await update.message.reply_text("✅ کانفیگ برای مشتری ارسال شد.")
            del pending_orders[replied_id]
        return
    ...
    info_msg = await context.bot.send_message(chat_id=ADMIN_ID, text=f"📥 رسید پرداخت جدید:\n\n👤 نام: {user.first_name}\n🆔 یوزرنیم: {username}\n🔢 آیدی: {user.id}\n📦 پلن: {plan}\n\n⬆️ برای ارسال کانفیگ، به این پیام ریپلای کن.")
