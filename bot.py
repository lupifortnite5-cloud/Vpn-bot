...
shop_open = True  # True = باز، False = بسته
...
def admin_keyboard():
status_btn = “🔴 بستن فروشگاه” if shop_open else “🟢 باز کردن فروشگاه”
...
if text == "📦 پلن‌ها":
    ...
    for p, price in plans.items():
        available = "✅ موجود" if configs.get(p) else "❌ ناموجود"
        msg += f"{p}: {price} تومان  {available}\n"
...
if text in plans:
    ...
    # بررسی موجودی بدون نمایش تعداد دقیق
    if not configs.get(text):
        await update.message.reply_text(f"❌ متأسفانه این پلن در حال حاضر موجود نیست.")
        return

    cart.setdefault(uid, {})
    cart[uid][text] = cart[uid].get(text, 0) + 1

    # بررسی اینکه تعداد درخواستی از موجودی بیشتر نشه
    if cart[uid][text] > len(configs[text]):
        cart[uid][text] -= 1
        await update.message.reply_text(f"❌ این پلن موجود نیست.")
        return
...
if data == "backup":
    try:
        ...
        await context.bot.send_document(
            ADMIN_ID,
            document=open("/tmp/configs_backup.json", "rb"),
...
if data == "toggle_shop":
    ...
    status = "🟢 باز شد" if shop_open else "🔴 بسته شد"
    await q.message.edit_text(
...
if data == "checkout":
    ...
    for plan, count in cart[uid].items():
        if len(configs.get(plan, [])) < count:
            await q.message.edit_text(
                f"❌ متأسفانه پلن زیر در حال حاضر موجود نیست:\n{plan}\n\nلطفاً از سبد حذف کن یا با پشتیبانی تماس بگیر."
            )
            return
