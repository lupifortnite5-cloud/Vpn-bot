...
def admin_keyboard():
btn = ‘🔴 بستن فروشگاه’ if state[‘shop_open’] else ‘🟢 باز کردن فروشگاه’
...
[InlineKeyboardButton(btn, callback_data=‘toggle_shop’)],
...
await update.message.reply_text(‘❌ فرمت اشتباه. مثال: /add1 vless://…’)
return
config = parts[1].strip()
map_cmd = {’/add1’: plan_list[0], ‘/add2’: plan_list[1], ‘/add3’: plan_list[2], ‘/add4’: plan_list[3], ‘/add5’: plan_list[4]}
...
await update.message.reply_text(‘❌ فرمت: /remove <index> <plan_index 0-4>’)
return
try:
i = int(context.args[0])
p = plan_list[int(context.args[1])]
if i < 0 or i >= len(configs[p]):
await update.message.reply_text(f’❌ ایندکس اشتباه. موجودی: {len(configs[p])}’)
return
...
```
if text == '📞 پشتیبانی':
    await update.message.reply_text(f'پشتیبانی: {SUPPORT_ID}')
    return

if text == '🏠 منو اصلی':
    await update.message.reply_text('🏠 منو اصلی', reply_markup=main_keyboard(uid, admin_id))
    return
...
if text == '🛒 خرید VPN':
    if not state['shop_open']:
        await update.message.reply_text('🔴 فروشگاه بسته است. با پشتیبانی تماس بگیر.')
        return
    kb = [[p] for p in plans]
...
if text == '🗑 سبد خرید':
    if uid not in cart or not cart[uid]:
        await update.message.reply_text('🛒 سبد خالی است.')
        return
    msg = '🛒 سبد خرید:\n\n'
    total = 0
    for p, c in cart[uid].items():
        msg += f'  {p} x{c} = {plans[p] * c} تومان\n'
        total += plans[p] * c
    msg += f'\n💰 مجموع: {total} تومان'
    kb = InlineKeyboardMarkup([
...
admin_id = context.bot_data[‘admin_id’]
if uid not in cart or not cart[uid]:
await update.message.reply_text(‘❌ سبد خالی است. ابتدا خرید کنید.’)
return
pending_orders[uid] = {
‘cart’: cart[uid].copy(),
...
msg_admin = f’📬 رسید از @{pending_orders[uid][“user”]}:\n\n’
for p, c in cart[uid].items():
msg_admin += f’  {p} x{c}\n’
...
await context.bot.send_photo(admin_id, photo=update.message.photo[-1].file_id, caption=msg_admin, reply_markup=kb_admin)
...
if data == 'continue_buy':
    await q.message.edit_text('پلن بعدی رو انتخاب کن.')
    return
if data == 'clear_cart':
    cart.pop(uid, None)
...
if data == 'stats':
    if uid != admin_id:
        return
    await q.message.edit_text(f'📊 آمار\nفروش: {len(sales)}\nسبدها: {len(cart)}\nدر انتظار: {len(pending_orders)}')
    return
...
if data == 'backup':
    if uid != admin_id:
        return
    try:
        bdata = json.dumps({'configs': configs, 'sales': sales}, ensure_ascii=False, indent=2)
...
if data == 'toggle_shop':
    ...
    await q.message.edit_text(f'وضعیت فروشگاه: {status}', reply_markup=admin_keyboard())
    return
...
if data.startswith('del|'):
    ...
    if uid in cart and plan in cart[uid]:
        ...
        if not cart[uid]:
            del cart[uid]
    await q.message.edit_text('🗑 از سبد حذف شد.')
    return

if data == 'checkout':
    if uid not in cart or not cart[uid]:
        await q.message.edit_text('سبد خالی است.')
        return
    if not state['shop_open']:
    ...
    for plan, count in cart[uid].items():
        if len(configs.get(plan, [])) < count:
            await q.message.edit_text(f'❌ پلن {plan} موجود نیست.')
            return
    total = 0
    msg = '🧾 سفارش:\n\n'
    for plan, count in cart[uid].items():
        msg += f'  {plan} x{count} = {plans[plan] * count} تومان\n'
...
if data.startswith('approve|'):
    ...
    for plan, count in order['cart'].items():
        if len(configs.get(plan, [])) < count:
            await q.message.edit_caption(f'❌ موجودی ناکافی: {plan}')
            return
        for _ in range(count):
            configs_to_send.append((plan, configs[plan].pop(0)))
    msg_user = '🎉 خریدت تایید شد!\n\n'
    ...
    try:
        ...
        await q.message.edit_caption(f'❌ خطا: {e}')
        for plan, cfg in configs_to_send:
    ...
    await q.message.edit_caption(f'\u2705 سفارش {order["user"]} ارسال شد.')
    return
...
if data.startswith('reject|'):
    ...
    try:
        await context.bot.send_message(target_uid, '❌ رسید رد شد. با پشتیبانی تماس بگیر.')
    except Exception as e:
